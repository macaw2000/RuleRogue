...
exports.handler = async (event) => {
  ...
  console.log('Real joinGame handler called for:', connectionId);
  ...
  try {
    ...
    if (!level1) {
      throw new Error('No level 1 data found in game');
    }
    ...
    // Create player - avoid using 'class' as property name
    const player = {
      id: connectionId,
      name: name || `Player${Date.now()}`,
      characterClass: characterClass, // Use characterClass instead of class
    ...
    const gameState = {
      ...
      players: players.map(p => ({
        id: p.playerId,
        name: p.name,
        class: p.characterClass || p.class || 'fighter',
    ...
    return {
      statusCode: 200,
      body: 'Joined real game'
    };
  } catch (error) {
    console.error('Real join game error:', error);
    
    await ws.sendToConnection(connectionId, {
      type: 'error',
      message: `Failed to join real game: ${error.message}`
    });
    
    return {
      statusCode: 500,
      body: 'Failed to join real game'
    };
  }
};