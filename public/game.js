...
let lastMoveTime = 0;
const moveDelay = 10; // 10ms between moves for ultra-fast responsive movement
...
function initWebSocket() {
  ...
  const wsUrl = "wss://pzaxtgam5c.execute-api.us-west-2.amazonaws.com/prod";
        ...
        if (player) {
          ...
          if (message.data.id !== myConnectionId) {
            ...
            if (message.data.x !== undefined && message.data.y !== undefined) {
              ...
              const distance =
                Math.abs(reconciledPos.x - currentPlayer.x) +
                Math.abs(reconciledPos.y - currentPlayer.y);
              if (distance > 0) {
                ...
                // Position reconciliation without logging
              }
            }
          }
          ...
          if (message.data.experience !== undefined)
            player.experience = message.data.experience;
          if (message.data.equippedWeapon !== undefined)
            player.equippedWeapon = message.data.equippedWeapon;
          if (message.data.equippedArmor !== undefined)
            player.equippedArmor = message.data.equippedArmor;

          // If this is the current player, update their reference for stats
          if (message.data.id === myConnectionId && currentPlayer) {
            if (message.data.hp !== undefined)
              currentPlayer.hp = message.data.hp;
            if (message.data.experience !== undefined)
              currentPlayer.experience = message.data.experience;
            if (message.data.equippedWeapon !== undefined)
              currentPlayer.equippedWeapon = message.data.equippedWeapon;
            if (message.data.equippedArmor !== undefined)
              currentPlayer.equippedArmor = message.data.equippedArmor;
            updatePlayerStats();
          }

          renderGame();

          // Check for items at player position after movement
          if (message.data.id === myConnectionId && currentPlayer) {
            checkForItemsAtPosition(currentPlayer.x, currentPlayer.y);
          }
        }
      }
      break;
...
// Check for items at player position and show message
function checkForItemsAtPosition(x, y) {
  if (
    !gameState ||
    !gameState.dungeon ||
    !gameState.dungeon[y] ||
    !gameState.dungeon[y][x]
  )
    return;
  ...
  if (
    ...
    displayMessage(
      `You see a ${itemType} here. Press ',' to pick it up.`,
      "yellow"
    );
  }
}
...
function isValidClientMove(x, y) {
  ...
  if (
    y < 0 ||
    y >= gameState.dungeon.length ||
    x < 0 ||
    x >= gameState.dungeon[0].length
  )
    return false;

  const cell = gameState.dungeon[y][x];
  // Allow optimistic movement on walkable tiles (floors, corridors, doors, stairs)
...
function updateCharacterPreview() {
  ...
  previewDiv.innerHTML = `
        <div class="${classData.color}">
            <strong>${playerName || "Your Character"} the ${
    classData.name
  }</strong><br>
...
function createCharacter() {
  ...
  if (!ws || ws.readyState !== WebSocket.OPEN) {
    initWebSocket();
    // Wait for connection then join - reduced delay for faster game start
    setTimeout(() => {
...
function hasLightSource(player) {
  // Check if player has an equipped light source
  if (
    player.equippedLight &&
    (player.equippedLight.type === "lantern" ||
      player.equippedLight.type === "lamp" ||
      player.equippedLight.type === "torch")
  ) {
    return true;
  }

  // Also check inventory for any light sources (temporary fix)
  if (player.inventory) {
    return player.inventory.some(
      (item) =>
        item.type === "lantern" || item.type === "lamp" || item.type === "torch"
    );
  }
...
// Calculate visible tiles using simple but effective line of sight
function calculateVisibleTiles(playerX, playerY, dungeon, viewRadius = 8) {
  const visible = new Set();
  const explored = getExploredTiles();

  // Always make the player's position visible
  visible.add(`${playerX},${playerY}`);

  // Simple ray casting - cast rays in all directions
  for (let angle = 0; angle < 360; angle += 2) {
    // Every 2 degrees for performance
    const rad = (angle * Math.PI) / 180;
    const dx = Math.cos(rad);
    const dy = Math.sin(rad);

    // Cast ray from player position outward
    for (let distance = 1; distance <= viewRadius; distance++) {
      ...
      // Stop at walls - can see the wall but not beyond it
      if (
        dungeon[y][x] === "-" ||
        dungeon[y][x] === "|" ||
        dungeon[y][x] === " "
  ...
  // Add visible tiles to explored tiles
  visible.forEach((tile) => explored.add(tile));
  saveExploredTiles(explored);

  return { visible, explored };
}

// Store explored tiles in localStorage per room
function getExploredTiles() {
  const roomCode = gameState?.roomCode || "DEFAULT";
  const level = currentPlayer?.level || 1;
  const key = `explored_${roomCode}_${level}`;
  const stored = localStorage.getItem(key);
  return stored ? new Set(JSON.parse(stored)) : new Set();
}

function saveExploredTiles(explored) {
  const roomCode = gameState?.roomCode || "DEFAULT";
  const level = currentPlayer?.level || 1;
  const key = `explored_${roomCode}_${level}`;
...
// Render the game with fog of war (NetHack style)
function renderGame() {
  if (!gameState || !gameState.dungeon) {
    console.log("renderGame: No game state or dungeon", {
      gameState: !!gameState,
      dungeon: !!gameState?.dungeon,
    });
    return;
  }
  console.log("renderGame: Rendering game", {
  ...
  // Calculate what the current player can see (NetHack-style fog of war)
  let visibilityData = { visible: new Set(), explored: new Set() };
  if (currentPlayer) {
    // Increase view radius if player has a light source
    const baseRadius = 8;
    const lightRadius = hasLightSource(currentPlayer) ? 12 : baseRadius;

    visibilityData = calculateVisibleTiles(
      currentPlayer.x,
      currentPlayer.y,
      dungeon,
      lightRadius
    ...
    // If no current player yet, show everything as visible (temporary until player loads)
    for (let y = 0; y < dungeon.length; y++) {
      for (let x = 0; x < dungeon[0].length; x++) {
        visibilityData.visible.add(`${x},${y}`);
      }
    }
  }
  ...
  // Add items to the display (only if visible or previously seen)
  items.forEach((item) => {
    const tileKey = `${item.x},${item.y}`;
    if (
      item.y >= 0 &&
      item.y < displayDungeon.length &&
      item.x >= 0 &&
      item.x < displayDungeon[0].length &&
      visibilityData.visible.has(tileKey)
    ) {
      // Only show items in line of sight
      displayDungeon[item.y][item.x] = item.symbol;
    }
  });

  // Add living monsters to the display (only if visible)
  monsters
    .filter((m) => m.hp > 0)
    .forEach((monster) => {
      const tileKey = `${monster.x},${monster.y}`;
      if (
        monster.y >= 0 &&
        monster.y < displayDungeon.length &&
        monster.x >= 0 &&
        monster.x < displayDungeon[0].length &&
        visibilityData.visible.has(tileKey)
      ) {
        // Only show monsters in line of sight
  ...
  players.forEach((player, index) => {
    if (
      player.y >= 0 &&
      player.y < displayDungeon.length &&
      player.x >= 0 &&
      player.x < displayDungeon[0].length
    ) {
  ...
  // Convert to string and display with colors and fog of war
  let dungeonHTML = "";
  for (let y = 0; y < displayDungeon.length; y++) {
    for (let x = 0; x < displayDungeon[y].length; x++) {
      const char = displayDungeon[y][x];
      const tileKey = `${x},${y}`;
      const isVisible = visibilityData.visible.has(tileKey);
      const isExplored = visibilityData.explored.has(tileKey);

      let className = "";
      let playerIndex = displayDungeon[y][x + "_playerIndex"];
      let playerData = displayDungeon[y][x + "_playerData"];

      // Skip rendering unexplored tiles (true NetHack style)
      if (!isVisible && !isExplored) {
        dungeonHTML += `<span class="unexplored"> </span>`;
        continue;
      }
      ...
      if (char === "@" || char === "*" || char === "&" || char === "+") {
        ...
        className = "wall"; // Walls (not walkable)
      } else if (char === "+") {
        className = "door"; // Doors (walkable)
      } else if (char === "#") {
        className = "corridor"; // Corridor floors (walkable)
      } else if (char === ".") {
        className = "floor"; // Room floors (walkable)
      } else if (char === " ") {
        className = "rock"; // Solid rock (not walkable)
      } else {
        className = "unknown";
      }
      ...
      if (!isVisible && isExplored) {
        className += " explored"; // Dimmed for explored but not visible
      } else if (isVisible) {
        className += " visible"; // Default visibility

        // Check if player has a light source equipped for full brightness
        if (currentPlayer && hasLightSource(currentPlayer)) {
          className += " lit"; // Full brightness with light source
        }
      }

      // Add tooltip data for players
      if (playerData && isVisible) {
        // Only show tooltips for visible players
        const isCurrentPlayer = playerData.id === myConnectionId;
        const tooltipText = `${playerData.name} the ${playerData.className}${
          isCurrentPlayer ? " (You)" : ""
...
function updatePlayerList() {
  ...
  gameState.players.forEach((player) => {
    ...
    const classData =
      characterClasses[player.class] || characterClasses.fighter;
    playerDiv.innerHTML = `
            <strong class="${classData.color}">${player.name}</strong><br>
            <span class="${classData.color}">${
      player.className || "Fighter"
    } (${player.symbol})</span><br>
...
function updatePlayerStats() {
  ...
  document.getElementById("dungeonLevel").textContent =
    ...
    if (currentPlayer.inventory.length === 0) {
      ...
      currentPlayer.inventory.forEach((item, index) => {
        ...
        if (
          ...
          if (item.type === "weapon") actionText = "wield";
          else if (item.type === "armor") actionText = "wear";
          else if (item.type === "lantern" || item.type === "lamp")
            actionText = "light";

          itemDiv.title = `Click to ${actionText} this ${item.type}`;
...
document.addEventListener("keydown", (event) => {
  ...
  switch (key) {
    ...
    case "arrowright":
      direction = "right";
      break;
    // NetHack diagonal movement (numpad style)
    case "7":
      direction = "up-left";
      break;
    case "9":
  ...
  if (direction) {
    ...
    // Ultra-responsive movement - no throttling for instant response
    const now = Date.now();
    ...
    if (currentPlayer && gameState) {
      ...
      if (isValidClientMove(newPos.x, newPos.y)) {
        ...
        const monster = gameState.monsters?.find(
          (m) => m.x === newPos.x && m.y === newPos.y && m.hp > 0
        );
...
document
  ...
  .addEventListener("change", updateCharacterPreview);
document
  .getElementById("playerName")
  .addEventListener("input", updateCharacterPreview);
...
function showInventory() {
  ...
  if (inventory.length === 0) {
    ...
    inventoryHTML =
      '<div style="margin-bottom: 15px;"><strong>Equipment:</strong><br>';
    inventoryHTML += `Weapon: ${
      currentPlayer.equippedWeapon ? currentPlayer.equippedWeapon.name : "none"
    }<br>`;
    inventoryHTML += `Armor: ${
      currentPlayer.equippedArmor ? currentPlayer.equippedArmor.name : "none"
    }<br>`;
    inventoryHTML += `Light: ${
      currentPlayer.equippedLight ? currentPlayer.equippedLight.name : "none"
    }</div>`;
    ...
    inventory.forEach((item, index) => {
      ...
      const isEquipped =
        (currentPlayer.equippedWeapon &&
          currentPlayer.equippedWeapon.id === item.id) ||
        (currentPlayer.equippedArmor &&
          currentPlayer.equippedArmor.id === item.id) ||
        (currentPlayer.equippedLight &&
          currentPlayer.equippedLight.id === item.id);
      if (isEquipped) {
        itemText += " (equipped)";
      }
    ...
    inventoryHTML += "</ul>";
    inventoryHTML +=
      '<p style="color: #888; font-size: 12px; margin-top: 10px;">Click on weapons/armor to equip them</p>';
  }
...
function equipItem(itemIndex) {
  if (
    ...
    itemIndex >= currentPlayer.inventory.length
  ) {
    return;
  }
  ...
  if (item.type === "weapon") {
    ...
    currentPlayer.equippedLight = item;
    displayMessage(`You light the ${item.name}.`, "yellow");
  } else {
...
function showEquipMenu() {
  ...
  const equippableItems = currentPlayer.inventory.filter(
    (item) =>
      item.type === "weapon" ||
      item.type === "armor" ||
      item.type === "lantern" ||
  ...
  let equipHTML =
    '<div style="margin-bottom: 15px;"><strong>Current Equipment:</strong><br>';
  equipHTML += `Weapon: ${
    currentPlayer.equippedWeapon ? currentPlayer.equippedWeapon.name : "none"
  }<br>`;
  equipHTML += `Armor: ${
  ...
  equippableItems.forEach((item, index) => {
    const actualIndex = currentPlayer.inventory.findIndex(
    ...
    equipHTML += `<li style="margin: 5px 0; cursor: ${
      isEquipped ? "default" : "pointer"
    }; color: ${color};" 
                          ${
                            isEquipped
                              ? ""
                              : `onclick="equipItem(${actualIndex}, '${item.type}'); closeEquipMenu();"`
                          }>
                          ${itemText}
                      </li>`;
  });
